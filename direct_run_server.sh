#!/bin/bash
# This is a minimal server script that uses pure bash for reliability
# If bash is available, this script should work

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting BCBS Values Platform Server"
echo "[$(date '+%Y-%m-%d %H:%M:%S')] ====================================="

# Function to create a simple HTML file if it doesn't exist
create_html_if_needed() {
    if [ ! -f "index.html" ]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Creating minimal index.html"
        cat > index.html << 'EOF'
<!DOCTYPE html>
<html>
<head>
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Creating server script"
cat > server.js << 'EOF'
...
# Start the server
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting Node.js server on port 5002"
node server.js